...
private:
    ...
    // can be modified in MODE::beats, and saved to that Track with enter key
    godot::PackedInt64Array proposed_beats;
    
    MODE mode { MODE::beats };
...
public:
    ...
    void _draw() override
    {
        ...
        switch( mode )
        {
            ...
            case MODE::notes:
            {
                draw_line({0, h/2 - now_line_height}, {w, h/2 - now_line_height}, notes_color, 1.5);
                draw_line({0, h/2 + now_line_height}, {w, h/2 + now_line_height}, notes_color, 1.5);
                break;
            }
